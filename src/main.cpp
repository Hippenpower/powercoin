...
#if defined(NDEBUG)
# error "Powercoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x8e3639bdeb7415c3f2e55ad7e26a9ffc616de46384adf56636076ddd6c5a2f5b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Powercoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Powercoin Signed Message:\n";
...
{
    // Powercoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Powercoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Powercoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Powercoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Powercoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Powercoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
       pchMessageStart[0] = 0xd2;
        ...
        hashGenesisBlock = uint256("0x8e3639bdeb7415c3f2e55ad7e26a9ffc616de46384adf56636076ddd6c5a2f5b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Power for your life";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x821328b32df124d2345da0a43c102dfd2a23c1421b6dfc48145a500625c803a4"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Powercoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PowercoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PowercoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PowercoinMiner(CWallet *pwallet)
{
    printf("PowercoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running PowercoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PowercoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PowercoinMiner, pwallet));
}
